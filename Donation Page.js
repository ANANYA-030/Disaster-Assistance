       ...
       {ItemsList.map((item) => {
        return (
          <div className='card' >
            ...
            <div>Quantity Required: {item.quantity}</div>
            <div>Amount per quantity: {item.amount}</div>    </div> )})} </div>
    <h2>Enter quantity you want to donate</h2>
    {ItemsList.map((items)=>{
        return(  <div> <h3>{items.name}:</h3>
          <input type="number"
                 onChange={(ev) => handleItemQuantity(ev.target.value, items)}
      autoComplete="off" />  </div>    ) })}
    <div>Total: {cartTotal}</div>
    <Link to="/Payment2"  state={{amtToPay:cartTotal}}
              >Proceed to donate</Link><br/> </div>  );  }
